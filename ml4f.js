(() => {
  ...
  function testFloatConv() {
    ...
    function test(v) {
      const u = float16toUInt16(v) & 65535;
      const v2 = float16AsUintToFloat(u);
      const d = Math.min(1e4 * Math.abs(v - v2), Math.abs((v - v2) / v));
      if (d > 2e-3) {
...
`;
  ...
  // src/testing.ts
  var tf3 = __toESM(__require("@tensorflow/tfjs"));
  ...
  async function runBrowser(seed2) {
    tf3.setBackend("cpu");
    const t0 = Date.now();
    seedRandom(seed2 || 220);
    testFloatConv();
    const sample = sampleModel("oneD");
    const float16weights = true;
    const optimize2 = false;
    const opts = { verbose: true, float16weights, optimize: optimize2 };
    logThumb(compileAndTest(sample, opts));
    await testAllModels({ verbose: false, optimize: optimize2 });
    console.log(Date.now() - t0 + "ms");
  }
  function getSampleModels() {
    return {
      id: [tf3.layers.inputLayer({
        inputShape: [10, 3, 1]
      })],
      conv2d: [tf3.layers.conv2d({
        inputShape: [50, 3, 1],
        kernelSize: [4, 4],
        filters: 16,
        strides: [1, 1],
      ...
      dspDense: [
        tf3.layers.inputLayer({ inputShape: [33] }),
        tf3.layers.dense({ units: 20, activation: "relu" }),
        tf3.layers.dense({ units: 10, activation: "relu" }),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      noDsp: [
        tf3.layers.inputLayer({ inputShape: [150] }),
        tf3.layers.reshape({ targetShape: [50, 3, 1] }),
        tf3.layers.conv2d({ filters: 16, kernelSize: 4, strides: 1, padding: "same", activation: "relu" }),
        tf3.layers.maxPooling2d({ poolSize: 2, strides: 2, padding: "same" }),
        tf3.layers.dropout({ rate: 0.1 }),
        tf3.layers.conv2d({ filters: 16, kernelSize: 2, strides: 1, padding: "same", activation: "relu" }),
        tf3.layers.maxPooling2d({ poolSize: 2, strides: 2, padding: "same" }),
        tf3.layers.flatten(),
        tf3.layers.dense({ units: 30, activation: "relu" }),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      tfjsGest: [
        tf3.layers.conv2d({
        ...
        tf3.layers.maxPooling2d({ poolSize: [2, 1], strides: [2, 1] }),
        tf3.layers.dropout({ rate: 0.1 }),
        tf3.layers.conv2d({
          kernelSize: [2, 1],
          filters: 16,
      ...
      oneD: [
        ...
        tf3.layers.maxPooling1d({ poolSize: [2] }),
        tf3.layers.dropout({ rate: 0.1 }),
        tf3.layers.conv1d({
          kernelSize: [2],
          strides: 1,
      ...
      avgPool: [
        ...
        tf3.layers.reshape({ targetShape: [50, 3, 1] }),
        tf3.layers.conv2d({ filters: 16, kernelSize: 4, strides: 1, padding: "same", activation: "relu" }),
        tf3.layers.avgPooling2d({ poolSize: 2, strides: 2, padding: "valid" }),
        tf3.layers.flatten(),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      avgPool2: [
        ...
        tf3.layers.reshape({ targetShape: [50, 3, 1] }),
        tf3.layers.conv2d({ filters: 16, kernelSize: 4, strides: 1, padding: "same", activation: "relu" }),
        tf3.layers.avgPooling2d({ poolSize: [2, 1], strides: [2, 1], padding: "valid" }),
        tf3.layers.flatten(),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      avgPool3: [
        tf3.layers.inputLayer({ inputShape: [150] }),
        tf3.layers.reshape({ targetShape: [50, 3, 1] }),
        tf3.layers.avgPooling2d({ poolSize: [8, 1], strides: [2, 1], padding: "valid" }),
        tf3.layers.flatten(),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      avgPool4: [
        tf3.layers.inputLayer({ inputShape: [2700] }),
        tf3.layers.reshape({ targetShape: [27, 1, 100] }),
        tf3.layers.avgPooling2d({ poolSize: [27, 1], strides: [27, 1], padding: "valid" }),
        tf3.layers.flatten(),
        tf3.layers.dense({ units: 3, activation: "softmax" })
      ],
      depth0: [
        tf3.layers.inputLayer({ inputShape: [15, 1, 1] }),
        tf3.layers.depthwiseConv2d({ kernelSize: [5, 1], depthMultiplier: 4, strides: 1, useBias: false })
      ],
      depth1: [
        tf3.layers.inputLayer({ inputShape: [213, 1, 15] }),
        tf3.layers.depthwiseConv2d({ kernelSize: [10, 1], depthMultiplier: 4, strides: 2, useBias: false })
      ],
      batch1: [
        tf3.layers.inputLayer({ inputShape: [213, 1, 15] }),
  ...
  function sampleModel(id) {
    const model = tf3.sequential();
    model.name = id;
    if (!_models)
      _models = getSampleModels();
    const layers2 = _models[id];
    if (!layers2) {
...
`);
  ...
  function evalModel(cres, data) {
    ...
    const confusion = range(dim).map((_) => range(dim).map((_2) => 0));
